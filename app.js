...
async function analyzeRandom() {
  ...
  // Use the Hugging Face inference client instead of calling the
  // deprecated api-inference endpoint. The InferenceClient is loaded
  // from a module in index.html and exposed on the global window. See
  // docs for details【979548302956284†L17-L20】【550930201269771†L520-L530】.
  const token = (els.token.value || '').trim();
  try {
    // Instantiate the client with or without an access token. Passing
    // undefined will allow unauthenticated requests on the free tier.
    const hf = new window.InferenceClient(token || undefined);
    // Perform sentiment analysis via text classification. The
    // classification method returns an array of candidate labels and
    // scores, e.g. [{ label: 'POSITIVE', score: 0.9 }, { ... }].
    ...
    // Different types of errors may be thrown by the inference client
    // (e.g. network errors, API errors). Provide a user-friendly
